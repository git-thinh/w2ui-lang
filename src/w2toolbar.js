    ...
    getItemHTML(item) {
        ...
        switch (item.type) {
            ...
            case 'drop': {
                ...
                html = `
                    <div id="tb_${this.name}_item_${item.id}" class="xtb-type xtb-type-${item.type} xtb-id-${item.id} ${classes.join(' ')} ${(item.class ? item.class : '')}"
                    ...
                    >
                        ...
                        ${ (text != '' && text != null) || item.count != null || arrow
                            ? `<div class="w2ui-tb-text" style="${item.type != 'label' ? (item.style ?? '') : ''}; ${!text ? 'padding-left: 0; margin-left: 23px;' : ''}">
                                    <span ${xlang(text)}>${w2utils.lang(text)}</span>                                    
                                    ${ item.count != null
                                        ? w2utils.stripSpaces(`
